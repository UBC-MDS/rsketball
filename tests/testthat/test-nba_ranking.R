...
test_ranking <- function(){
  ...
  test_that("The function is allowing top value to be non-numerical",
            expect_error(nba_ranking(tibble::tibble(ranked = c("A", "B", "C"),
                                                by = c(3, 2, 1)),
  ...
  # Testing the warning
  test_that("The function is allowing other values as data input. Needs tibble or data frame",
            expect_warning(nba_ranking(tibble::tibble(ranked = c("1", "2", "3"), by = c(3, 2, 1)), ranked, by, 15, TRUE, mean)))

  test_that('Plot should use a geom_col and a geom_text', {
    p <- nba_ranking(tibble::tibble(ranked = c("A", "B", "C"), by = c(3, 2, 1)), ranked, by, 2, TRUE, mean)
